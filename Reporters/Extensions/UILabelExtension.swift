...
extension UILabel {
    ...
    public func decideTextDirection() {
        let tagScheme = [NSLinguisticTagScheme.language]
        let tagger = NSLinguisticTagger(tagSchemes: tagScheme, options: 0)
        tagger.string = self.text
        if let lang = tagger.tag(at: 0, scheme: .language, tokenRange: nil, sentenceRange: nil),  lang.rawValue.hasPrefix("he") || lang.rawValue.hasPrefix("ar") {
